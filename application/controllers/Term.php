...
class Term extends CI_Controller
{
	...
	{
		$config = array();
		$config["base_url"] = base_url() . "term/index";

		$this->pagination->initialize($config);

		$data['title'] = display('term');
		#-------------------------------#
		...
		$data['section'] = $this->home_model->section('term');
		$data['parent_menu'] = $this->menu_model->get_parent_menu();
		$data["links"] = $this->pagination->create_links();
		$data['content'] = $this->load->view('website/terms', $data, true);

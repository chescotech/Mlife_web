...
class Resetpass extends CI_Controller
{
	...
	{
		$config = array();
		$config["base_url"] = base_url() . "Resetpass/index";

		$this->pagination->initialize($config);

		$data['title'] = display('Resetpass');
		#-------------------------------#
		...
		$data['section'] = $this->home_model->section('resetpass');
		$data['parent_menu'] = $this->menu_model->get_parent_menu();
		$data["links"] = $this->pagination->create_links();
		$data['content'] = $this->load->view('website/resetpass', $data, true);
